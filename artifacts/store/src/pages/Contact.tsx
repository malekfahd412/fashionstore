...
export default function Contact() {
  ...
  const socials = [
    { key: "social_instagram", label: "IG", href: settings.social_instagram },
    { key: "social_facebook", label: "FB", href: settings.social_facebook },
    { key: "social_twitter", label: "TW", href: settings.social_twitter },
    { key: "social_tiktok", label: "TK", href: settings.social_tiktok },
  ].filter(s => s.href);
          ...
          <div>
            ...
            {contactItems.length > 0 ? (
              ...
              <p className="text-muted-foreground text-sm">
                {isAr ? "يمكنك التواصل معنا عبر النموذج أدناه." : "Please use the form to get in touch."}
              </p>
            )}
          </div>
          ...
          {socials.length > 0 && (
            <div>
              ...
              <div className="flex gap-3">
                {socials.map((s) => (
                  <a key={s.key} href={s.href} target="_blank" rel="noopener noreferrer" className="w-10 h-10 border border-border flex items-center justify-center hover:bg-primary hover:text-primary-foreground hover:border-primary transition-colors text-xs font-medium">
                    {s.label}
                  </a>
                ))}
              </div>
