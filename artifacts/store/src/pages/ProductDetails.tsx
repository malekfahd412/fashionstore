...
export default function ProductDetails() {
  ...
  return (
    <div className="bg-background" style={{ fontFamily: "'Inter', sans-serif" }}>
              ...
              <div className="relative">
                ...
                {showSharePanel && (
                  <div className="absolute end-0 top-full mt-2 bg-background border border-border shadow-lg p-4 w-60 z-20">
                    ...
                    <a
                      href={`https://x.com/intent/tweet?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(name)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="mt-1.5 w-full flex items-center gap-3 py-2.5 px-3 border border-border hover:bg-secondary transition-colors text-xs text-foreground"
                    >
                      <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.253 5.622L18.244 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                      X (Twitter)
                    </a>
                  </div>
                )}
      ...
      {/* Footer visibility sentinel — hides sticky CTA when footer enters view */}
      <div ref={footerSentinelRef} aria-hidden="true" />

      {/* ── Sticky Mobile CTA ─────────────────────────────────────────────── */}
      <div
        className={`fixed bottom-0 left-0 right-0 z-40 lg:hidden transition-transform duration-300 ${showStickyBar && !footerVisible ? "translate-y-0" : "translate-y-full"}`}
